...
with tab1:
    ...
    with st.form("add form", clear_on_submit=True):
        ...
        if add_btn:
            new_row = {"Record ID": str(uuid.uuid4()),"Project ID": project_id, "Project Name": project_name,"Employee ID": emp_id, 
                "Employee Name": emp_name,"Job Undertaken": job,"Time Duration": duration,"Progress": progress,"Last Updated": datetime.now()}
...
with tab2:
    ...
    if project_df.empty:
        ...
        for _, row in project_df.iterrows():
            ...
            with col1:
                st.write(f"**{row['Employee Name']}** "f"({row['Employee ID']})")
        ...
        chart = alt.Chart(project_df).mark_bar().encode(
            ...
            tooltip=[ "Employee Name", "Employee ID", "Job Undertaken", "Progress"]).properties(height=450)
